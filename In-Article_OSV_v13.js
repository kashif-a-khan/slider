...
function injectInArticle() {
  ...
  // 🎯 1. Target OSV containers first
  const targetId = isMobile ? "osv_player_wap" : "osv_player_web";
  const targetDiv = doc.getElementById(targetId);

  if (targetDiv) {
    targetDiv.innerHTML = ""; // optional: clear existing content
    targetDiv.appendChild(container);
    return;
  }

  // 🎯 2. European Markets (your existing priority)
