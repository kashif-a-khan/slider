(function () {
  ...
  function requestAds(isMidroll) {
    ...
    req.adTagUrl = waterfall[index] + "&description_url=" + PAGE_URL + "&vid_t=" + encodeURIComponent(document.title) + "&vid_kw=" + encodeURIComponent("stock market, investing, finance, crude oil price, gold price, rupees, sensex, bse, share market") + "&vid_d=" + Math.floor(video.duration || 632) +"&vpmute=" + getVPMute() + "&cust_params=" + encodeURIComponent("category=finance" + "&section=markets" + "&content=stocks" + "&format=video" + "&lang=en") + "&correlator=" + Date.now();
