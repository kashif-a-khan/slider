(function () {
  ...
  // ─── HLS dedup guard ────────────────────────────────────────────────────────
  // FIX: Prevents HLS.js from being injected into <head> more than once
  // if loadContentVideo() is ever called multiple times.
  ...
  // ─── Inject button transition styles into <head> once ────────────────────────
  // Keeps hover/transition logic in CSS rather than inline JS manipulation.
  ...
  uiStyles.textContent = `
    ...
    /* Show overlay on container hover — but not during ads */
  ...
  // ─── Mute button: removed conflicting display:none + display:flex ────────────
  // FIX: The original had both `display:none` and `display:flex` on the same
  // element, making the mute button always visible on render. Now starts hidden
  // via a JS-controlled style; display:flex is applied when video begins playing.
  container.innerHTML = `
    ...
    <!-- Center play/pause overlay (appears on hover, hidden during ads) -->
    <button id="mc-playpause-overlay" aria-label="Play / Pause">
      <svg id="mc-pp-icon" xmlns="http://www.w3.org/2000/svg" width="${isMobile ? 22 : 26}" height="${isMobile ? 22 : 26}" viewBox="0 0 24 24" fill="white">
        <!-- pause icon by default; JS swaps to play when paused -->
        <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/>
      </svg>
    </button>

    <!-- Top-left: close button (shown only when floating) -->
  ...
  // ─── getInlineTop: recalculate when floating ─────────────────────────────────
  // FIX: Original only recalculated when !isFloating, so a resize while floating
  // left cachedInlineTop stale. Now recalculates unconditionally after a resize.
  function getInlineTop() {
    if (cachedInlineTop === null) {
      const wasFloating = isFloating;

      // Temporarily restore position to measure true document offset
      if (wasFloating) {
        placeholder.style.display = "block";
      }

      // Use placeholder offset when floating (container is fixed-positioned)
  ...
  window.addEventListener("resize", () => {
    // FIX: Null-out cache on resize so it's always fresh on next scroll check,
    // regardless of whether the player is currently floating or inline.
    cachedInlineTop = null;
  });

  // ─── Scroll handler: throttled via rAF ───────────────────────────────────────
  // FIX: Original fired on every scroll event. Now batched with rAF to avoid
  // layout thrash and improve performance especially on mobile.
  ...
  function loadContentVideo() {
    ...
    // FIX: Guard against double-injection of HLS.js script into <head>
    ...
    // FIX: Handle HLS script load failure gracefully; reset guard so retry is possible
  ...
  function requestAds(isMidroll) {
    ...
    // ─── Reset adsManagerReady before each new request ─────────────────────────
    // FIX: adsManagerReady was never cleared between ad requests, so a stale or
    // destroyed manager could be re-started by startAdsIfViewable() prematurely.
  ...
  // ─── tryNextVast: takes explicit context instead of relying on isPreroll state ─
  // FIX: Original used isPreroll closure state which could be stale at error time.
  // Now the function receives explicit context about which waterfall errored.
  function tryNextVast(isPrerollContext) {
    ...
    if (isPrerollContext) {
      ...
      if (prerollIndex < PREROLL_WATERFALL.length) requestAds(false);
        ...
        // All prerolls exhausted — fall through to content
        isPreroll = false;
        video.play().catch(() => {});
      }
    } else {
  ...
  function initIMA() {
    ...
    // FIX: Pass a bound closure so tryNextVast knows which waterfall context
    // triggered the error, rather than guessing from shared isPreroll state.
  ...
  // ─── IMA SDK load failure handling ───────────────────────────────────────────
  // FIX: Original had no onerror on the IMA script tag. If the SDK fails (e.g.
  // ad blocker), the player now falls back gracefully to content playback.
  function loadIMA() {
  ...
  if (ppOverlay) {
    ppOverlay.addEventListener("click", () => {
      ...
      void ppOverlay.offsetWidth; // reflow to restart animation
  ...
  // ─── Show mute button correctly on play ──────────────────────────────────────
  // FIX: Original mute button CSS had conflicting display:none + display:flex
  // which made it always visible. Now explicitly set display:flex here on play.
  ...
  function onAdsManagerLoaded(e) {
    ...
    adsManager.addEventListener(google.ima.AdEvent.Type.STARTED, () => {
      adPlaying = true;
      container.setAttribute("data-ad-playing", "true"); // hides play/pause overlay
      if (thumbnail) thumbnail.style.display = "none";
      muteBtn.style.display = "flex";
      video.pause();
    ...
    adsManager.addEventListener(google.ima.AdEvent.Type.ALL_ADS_COMPLETED, () => {
      adPlaying = false;
      container.removeAttribute("data-ad-playing"); // re-enables play/pause overlay
      adLayer.style.pointerEvents = "none";
  ...
  closeBtn.onclick = () => {
    playerKilled = true;
    // FIX: Removed redundant optional chaining + try/catch combination.
    // Using try/catch alone is sufficient and consistent for both cleanup calls.
