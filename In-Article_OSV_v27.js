(function () {
  ...
  // IMPROVEMENT 1: Replaced hlsScriptLoading boolean + raw onload callback with
  // a Promise-based loader that deduplicates concurrent calls and handles the case
  // where HLS.js is already loaded (e.g. disk cache hit).
  let hlsReady = null;
  let imaReady = null;
  ...
  // IMPROVEMENT 1: Promise-based HLS loader. Deduplicates concurrent calls,
  // handles disk cache hits where Hls is already on window, and re-uses
  // the same script element if one was already injected.
  ...
  // IMPROVEMENT 2: Promise-based IMA loader. Same deduplication pattern —
  // safe to call multiple times, resolves instantly on cache hit.
  ...
  // IMPROVEMENT 3: HLS preload hint. Tells the browser to start fetching
  // the m3u8 manifest in parallel with script execution, reducing
  // time-to-first-frame once HLS.js attaches.
  if (CONTENT_VIDEO.includes(".m3u8")) {
  ...
  // IMPROVEMENT 4 (midroll recovery): Reset midrollPlaying before retrying
  // the next waterfall entry, not only when the waterfall is exhausted.
  // Previously, if entry N+1 also failed, midrollPlaying stayed true forever
  // and no further midrolls would trigger for that session.
  ...
  // IMPROVEMENT 2 (safe reinitialization): Replaced the raw script.onload +
  // window.google.ima check with ensureIMAReady() Promise. This deduplicates
  // the SDK load, handles disk cache hits, and feeds into a clean .catch()
  // for the preroll fallback (Improvement 5).
  function loadIMA() {
    ensureIMAReady()
      .then(initIMA)
      .catch(() => {
        // IMPROVEMENT 5 (preroll fallback): Also destroy hlsInstance cleanly
        // before falling back to content, matching V1.3 behaviour.
